...
img = read_img('Dragon.png')
line_wdt = 9
blur_value = 7
totalColors = 4

edgeImg = edge_detection(img, line_wdt, blur_value)
Cimg = color_quantisation(img, totalColors)
blurred = cv2.bilateralFilter(Cimg, d=7, sigmaColor=200, sigmaSpace=200)
cartoon_img = cv2.bitwise_and(blurred, blurred, mask=edgeImg)
cv2.imwrite('dragon.jpg', cartoon_img)